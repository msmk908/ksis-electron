...
import { SIGNAGE_INVENTORY, SIGNAGE_GRID } from "../../constants/page_constant";
...
export function ToggleSwitch() {
  ...
  const [isChecked, setIsChecked] = useState(
    location.pathname === SIGNAGE_INVENTORY
  );
  ...
  const handleToggle = () => {
    ...
    // 상태 변경 후 해당 페이지로 이동
    if (newChecked && location.pathname !== SIGNAGE_INVENTORY) {
      navigate(SIGNAGE_INVENTORY); // 페이지 A로 이동
    } else if (!newChecked && location.pathname !== SIGNAGE_GRID) {
      navigate(SIGNAGE_GRID); // 페이지 B로 이동
    }
  };
